module neuron_1 
    #(
        parameter NUM_INPUTS = 8,            // 1 to 8 inputs
        parameter signed [31:0] WEIGHTS [8] = '{8{0}},  // Weight array
        parameter signed [31:0] BIAS = 0,    // Neuron bias
        parameter signed [31:0] V_TH = 0     // Firing threshold
    )
    (
        input  clk,
        input  reset,
        input  [NUM_INPUTS - 1 :0] spikes_i,          // Fixed 8-bit input (unused bits ignored)
        input  neuron_reset,
    ...
    reg signed [31:0] weighted_sum[1:0];  // Two pipeline stages for summing
